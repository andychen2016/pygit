...
def read_index():
    ...
    while idx + 62 < len(entry_data):
        ...
        path_end = entry_data.index(b'\x00', field_end)
...
def diff():
    '''show diff of files changed between index and copy'''
...
def commit(message, auther=None):
    ...
    if auther is None:
        auther = '{} <{}>'.format(os.environ['GIT_AUTHER_NAME'], os.environ('GIT_AUTHER_EMAIL'))

    timestamp = int(time.mktime(time.localtime()))
    utc_offset = -time.timezone
    auther_time = '{} {}{:02}{:02}'.format(
            timestamp,
    ...
    lines.append('author {} {}'.format(auther, auther_time))
    lines.append('committer {} {}'.format(auther, auther_time))
    lines.append('')
...
def extract_lines(data):
    ...
    for _ in range(10000):
        line_length = int(data[idx:idx + 4], 16)
        line = data[idx + 4:idx + line_length]
        lines.append(line)
        if line_length == 0:
            idx += 4
        else:
...
def find_tree_objects(tree_sha1):
    ...
    """
    objects = {tree_sha1}
    for mode, _, sha1 in read_tree(sha1=tree_sha1):
...
if __name__ == '__main__':
    ...
    sub_parser.add_argument('git_url',
            help='URL of git repo, eg: https://github.com/andychen2016/pygit.git')
    ...
    if args.command == 'add':
        ...
        commit(args.message, auther=args.author)
